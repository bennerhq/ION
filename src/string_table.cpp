...
StringLiteralTable::StringLiteralTable(int64_t base_cursor)
    : base_cursor_(base_cursor), data_cursor_(base_cursor), heap_start_(base_cursor) {}
...
const std::unordered_map<std::string, int64_t> &StringLiteralTable::Offsets() const {
    return string_offsets_;
}

const std::vector<std::pair<int64_t, std::string>> &StringLiteralTable::Segments() const {
    return data_segments_;
}

int64_t StringLiteralTable::HeapStart() const {
    return heap_start_;
}
...
int64_t StringLiteralTable::Align8(int64_t value) {
    return TypeLayout::Align8(value);
}
