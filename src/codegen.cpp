...
private:
  ...
  void RequireSameType(const std::shared_ptr<Type> &expected,
                       const std::shared_ptr<Type> &actual, int line);
  void EmitStructInit(const StructDef &def);
};

struct CodeGen::EmitterRegistry {
  using ExprEmitter =
      std::function<std::shared_ptr<Type>(const ExprPtr &, Env &)>;
  using ExprInferrer =
      std::function<std::shared_ptr<Type>(const ExprPtr &, Env &)>;
  ...
  using WasmTypeEmitter =
      std::function<std::string(const std::shared_ptr<Type> &)>;
  ...
  using AssignmentFn =
      std::function<void(const ExprPtr &, const ExprPtr &, Env &)>;
  ...
  using FunctionInfoGetter =
      std::function<const FunctionInfo *(const std::string &)>;

  explicit EmitterRegistry(CodeGen &cg)
      : out_(cg.out_), string_offsets_(cg.string_table_.Offsets()),
        emit_expr_([&cg](const ExprPtr &expr, Env &env) {
          return cg.EmitExpr(expr, env);
        }),
        infer_expr_([&cg](const ExprPtr &expr, Env &env) {
          return cg.InferExprType(expr, env);
        }),
        resolve_type_(
            [&cg](const TypeSpec &spec) { return cg.ResolveType(spec); }),
        type_size_([&cg](const std::shared_ptr<Type> &type) {
          return cg.TypeSize(type);
        }),
        emit_load_(
            [&cg](const std::shared_ptr<Type> &type) { cg.EmitLoad(type); }),
        wasm_type_([&cg](const std::shared_ptr<Type> &type) {
          return cg.WasmType(type);
        }),
        emit_zero_(
            [&cg](const std::shared_ptr<Type> &type) { cg.EmitZero(type); }),
        require_same_type_(
            [&cg](const std::shared_ptr<Type> &expected,
                  const std::shared_ptr<Type> &actual,
                  int line) { cg.RequireSameType(expected, actual, line); }),
        emit_assignment_(
            [this](const ExprPtr &target, const ExprPtr &expr, Env &env) {
              MakeAssignmentEmitter().EmitAssignment(target, expr, env);
            }),
        emit_condition_([this](const ExprPtr &expr, Env &env) {
          MakeConditionEmitter().EmitCondition(expr, env);
        }),
        emit_stmt_([this](const StmtPtr &stmt, Env &env) {
          MakeStmtEmitter().EmitStmt(stmt, env);
        }),
        emit_struct_init_call_([&cg](const std::string &name) {
          cg.out_ << "    call $init_" << name << "\n";
        }),
        emit_function_([&cg](const Function &fn, const std::string &owner) {
          cg.EmitFunction(fn, owner);
        }),
        emit_struct_init_(
            [&cg](const StructDef &def) { cg.EmitStructInit(def); }),
        get_function_info_(
            [this](const std::string &name) -> const FunctionInfo * {
              auto it = functions_.find(name);
              return it == functions_.end() ? nullptr : &it->second;
            }),
        functions_(cg.functions_), structs_(cg.structs_), program_(cg.program_),
  ...
  ExprAccessEmitter &MakeAccessEmitter() {
    if (!access_emitter_) {
      access_emitter_ = std::make_unique<ExprAccessEmitter>(
          out_, emit_expr_, infer_expr_, resolve_type_, type_size_, emit_load_,
          [this](const std::string &name) -> const FunctionInfo * {
            auto it = functions_.find(name);
            return it == functions_.end() ? nullptr : &it->second;
          },
  ...
  ExprNewEmitter &MakeNewEmitter() {
    if (!new_emitter_) {
      new_emitter_ = std::make_unique<ExprNewEmitter>(
          out_, emit_expr_, resolve_type_, type_size_,
          [this](const std::string &name) -> const StructInfo * {
            auto it = structs_.find(name);
            return it == structs_.end() ? nullptr : &it->second;
          });
  ...
  ::AssignmentEmitter &MakeAssignmentEmitter() {
    if (!assignment_emitter_) {
      assignment_emitter_ = std::make_unique<::AssignmentEmitter>(
          out_, emit_expr_, type_size_, require_same_type_,
          [this](const std::string &name) -> const StructInfo * {
            auto it = structs_.find(name);
            return it == structs_.end() ? nullptr : &it->second;
          },
  ...
  ::ConditionEmitter &MakeConditionEmitter() {
    if (!condition_emitter_) {
      condition_emitter_ =
          std::make_unique<::ConditionEmitter>(out_, emit_expr_);
    }
  ...
  ::StmtEmitter &MakeStmtEmitter() {
    if (!stmt_emitter_) {
      stmt_emitter_ = std::make_unique<::StmtEmitter>(
          out_, emit_expr_, emit_assignment_, emit_condition_, emit_zero_,
          require_same_type_);
    }
  ...
  ::ModuleEmitter &MakeModuleEmitter() {
    if (!module_emitter_) {
      module_emitter_ = std::make_unique<::ModuleEmitter>(
          out_, program_, strings_, emit_function_, emit_struct_init_,
          get_function_info_);
    }
...
CodeGen::CodeGen(const Program &program,
                 const std::unordered_set<std::string> &type_names)
    : program_(program) {
  (void)type_names;
}
...
std::string CodeGen::Generate() {
  ...
  for (auto &pair : functions_) {
    ...
    Env env;
    size_t param_idx = 0;
    if (info.decl->is_method) {
      param_idx = 1; // Skip 'this' in info.params
      env.current_struct = info.decl->owner;
    }

    for (size_t i = 0; i < info.decl->params.size(); ++i) {
      std::string name = info.decl->params[i].second;
      env.params[name] = LocalInfo{name, info.params[i + param_idx]};
      env.locals[name] = LocalInfo{name, info.params[i + param_idx]};
    }
    checker.Check(info.decl->body, env);
  }
...
void CodeGen::EmitFunction(const Function &fn, const std::string &owner) {
  FunctionInfo &info =
      functions_.at(owner.empty() ? fn.name : owner + "." + fn.name);
...
std::shared_ptr<Type> CodeGen::InferExprType(const ExprPtr &expr, Env &env) {
  ...
  if (expr->type)
    return expr->type;
  throw CompileError("Type not checked for expression at line " +
                     std::to_string(expr->line));
}
